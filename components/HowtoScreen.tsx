...
const RANKS = [
  ...
  { r: "A", d: "85%以上", c: "#d8a23a" },
  { r: "B", d: "70%以上", c: "var(--muro)" },
  { r: "C", d: "50%以上", c: "#caa86d" },
  { r: "D", d: "それ未満", c: "#cfc4ac" },
];
    ...
    <div className="relative min-h-dvh px-5 py-7">
      ...
      <div className="mx-auto mt-1 max-w-md">
        <WoodPanel className="px-6 py-3 text-center">
          <p className="font-mincho text-xs tracking-[0.3em] text-[#f8f1e0]/90 [text-shadow:0_1px_2px_rgba(0,0,0,0.6)]">
            ― 鐘楼の心得 ―
          </p>
          <h2 className="font-brush text-3xl text-[#f8f1e0] [text-shadow:0_2px_3px_rgba(0,0,0,0.7)]">
            遊び方
          </h2>
        </WoodPanel>
      </div>
      ...
      <div className="mx-auto mt-5 max-w-2xl space-y-4">
        {/* 導入：住職 */}
        <WoodPanel className="flex items-center gap-4 p-4">
          <div className="float-y w-24 shrink-0 sm:w-28">
            <SafeImage
              ...
              width={320}
              height={320}
              className="h-auto w-full drop-shadow-[2px_4px_6px_rgba(45,27,0,0.45)]"
        ...
        <div className="space-y-3">
          {STEPS.map((s) => (
            <WoodPanel key={s.n} className="flex items-center gap-3 p-3">
              <span className="font-brush grid h-11 w-11 shrink-0 place-items-center rounded-full border-2 border-[#f8f1e0]/70 bg-ink text-xl text-washi">
                {s.n}
              </span>
              <div>
                <p className="font-brush text-xl text-ink">{s.title}</p>
                <p className="font-mincho text-sm leading-relaxed text-ink/80">
                  {s.body}
                </p>
              </div>
            </WoodPanel>
        ...
        <WoodPanel className="p-4">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex-1 rounded-md border-2 border-muro/70 bg-washi/85 p-3 text-center">
            ...
            <div className="flex-1 rounded-md border-2 border-bonnou/70 bg-washi/85 p-3 text-center">
              <p className="font-brush text-2xl text-bonnou">煩悩 →</p>
              <p className="font-mincho text-xs text-ink-sub">右へ・鐘を鳴らす</p>
            </div>
          </div>
          <p className="font-mincho mt-3 text-center text-xs text-ink/70">
            ※札を左右にドラッグしてもOK
          </p>
        </WoodPanel>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <WoodPanel className="p-4">
            ...
            <p className="font-mincho mb-2 text-xs text-ink/70">
              無私・善行・感謝など
            </p>
            <ul className="space-y-1.5">
              {MURO_EX.map((t) => (
                <li
                  key={t}
                  className="font-mincho rounded border border-muro/40 bg-washi/90 px-2 py-1 text-sm text-ink"
                >
                  「{t}」
                </li>
              ))}
          ...
          <WoodPanel className="p-4">
            ...
            <ul className="space-y-1.5">
              {BONNOU_EX.map((t) => (
                <li
                  key={t}
                  className="font-mincho rounded border border-bonnou/40 bg-washi/90 px-2 py-1 text-sm text-ink"
                >
                  「{t}」
                </li>
              ))}
        ...
        <WoodPanel className="p-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {RANKS.map((x) => (
              <div
                key={x.r}
                className="flex items-center gap-2 rounded-full border-2 bg-washi/90 px-3 py-1"
